...
def masks_init_filling(
    ...
    # masks.shape: (30, 1, 128, 128)
    # masks == 1.表示将masks中为1的地方标为Ture，其他地方为False，返回的是一个布尔矩阵。
    # seq.shape: (30, 4, 128, 128)
    # (masks == 1.).expand_as(seq)表示将布尔矩阵扩展到与seq维度一致，即flag.shape为(30, 4, 128, 128)。
    # 所以这里的flag是一个维度为(30, 4, 128, 128)的布尔矩阵。

    if fill_type == 'fill_value':
        masked_seq[flag] = fill_value # 将masked_seq中对应于布尔矩阵flag中为True的位置的值填补为1。
