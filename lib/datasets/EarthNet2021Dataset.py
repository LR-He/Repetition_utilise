...
class EarthNet2021Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        if isinstance(filter_settings, dict):
            # OmegaConf.create()可以将字典变成一个OmegaConf配置对象，结构和字典结构相同。
            # 可以使用OmegaConf配置对象来访问和修改配置值，使得配置更容易管理，特别是当配置文件非常复杂或嵌套时。
            # OmegaConf 配置对象的类型是 omegaconf.omegaconf.DictConfig，它是 OmegaConf 库中的一部分。
            # 即OmegaConf.create()返回的对象类型是DictConfig。
        ...
        if filter_settings and filter_settings.get('type', None) is not None:
            # variable_seq_length表示可变序列长度，指序列的长度是否可以根据需要进行调整或变化。
        ...
        if mask_kwargs is not None:
            # mask_kwargs.get('mask_type', 'random_clouds') 使用字典中的 get 方法来获取键 'mask_type' 对应的值。
            # 如果字典中存在 'mask_type' 键，它将返回与该键相关联的值。如果字典中没有 'mask_type' 键，它将返回默认值 'random_clouds'。
            ...
            self.fixed_masking_ratio = mask_kwargs.get('fixed_masking_ratio', False) # 这里‘fixed_masking_ratio’为False，表示将随机变化遮挡比率。
            self.intersect_real_cloud_masks = mask_kwargs.get('intersect_real_cloud_masks', False)  # 这里‘intersect_real_cloud_masks’为False，表示忽略实际云遮挡数据。
            self.dilate_cloud_masks = mask_kwargs.get('dilate_cloud_masks', False) # 这里‘dilate_cloud_masks’为False，表示不扩大云掩膜的范围。
        ...
        if '-nir' in self.channels:
            ...
            self.s2_channels += [3] # 拼接list，即在原始list最后面加上一个值‘3’。
        else:
        ...
        # -------------------------------------- List of samples -------------------------------------- #
        # 在计算机领域，"verbose" 通常表示在程序或系统中提供详细信息或输出的选项或功能。
        # 通常，当程序以 verbose 模式运行时，它会生成比正常运行模式更多的信息，这些信息通常用于调试、故障排除或记录程序执行的详细过程。
        # verbose 输出通常包括变量值、步骤、操作的详细描述等，以帮助开发人员或用户更好地理解程序的行为。
        # 在许多命令行工具、编程语言和库中，你可能会看到 "--verbose" 或 "-v" 等选项，以启用 verbose 模式，以便获取更多的信息来跟踪程序的执行。
        # 这对于识别和修复错误以及监视程序性能非常有用。
        self.verbose = verbose

        # Open hdf5 file
        # 引用一个HDF5文件，允许在代码的其他部分使用这个对象来读取和操作文件中的数据。
        # 这个文件对象代表了指定路径上的HDF5文件，使用只读模式 'r' 打开，同时启用了 SWMR（Single-Writer Multi-Reader）模式以支持多个读取器同时访问文件。
        self.f = h5py.File(self.hdf5_file, 'r', libver='latest', swmr=True)
        # 这行代码是Python代码，它使用h5py库中的h5py.File函数打开一个HDF5文件，以便后续可以读取和操作HDF5文件中的数据。
        # 具体解释如下：
        # 1. self.f: 这是一个类成员变量（成员属性），用于在类的其他方法中引用打开的HDF5文件对象。通常，self.f会在类的构造函数中打开，并在类的其他方法中使用。
        # 2. h5py.File: 这是h5py库中的函数，用于打开HDF5文件。它的第一个参数是要打开的HDF5文件的路径，即self.hdf5_file，它存储了文件的路径信息。
        # 3. 'r': 这是文件打开模式，表示以只读模式打开文件。这意味着你可以从文件中读取数据，但不能修改文件的内容。
        # 4. libver='latest': 这是一个附加参数，用于指定HDF5文件版本。'latest'表示使用最新的HDF5文件版本。不同的HDF5文件版本支持不同的特性和性能，因此你可以根据需要选择特定的版本。
        # 5. swmr=True: 这是另一个附加参数，表示启用"Single-Writer Multi-Reader"（SWMR）模式。SWMR模式允许多个读取器同时访问文件，而只有一个写入器可以修改文件。这对于在多个进程或线程中并行访问HDF5文件时非常有用。
        # 总之，这行代码的作用是打开指定路径的HDF5文件以供读取，而且支持多个读取器同时访问文件的SWMR模式。一旦文件被打开，你可以使用self.f对象来访问文件中的数据集和其他HDF5数据。
    ...
    def __getitem__(
        ...
        if not self.preprocessed or (self.split == 'train' and self.mode in ['train', 'all']):
            ...
            # H x W x C x T
            # 加载整个S2图像时间序列，以便对无云和有云的图像应用相同的数据增强。
            ...
            # 将时间序列中云量大于 render_occluded_above_p 的图像认为是被云全遮盖的图像，将其掩膜值全设置为1，表示所有像素点都被云遮盖。
        ...
        out = {
            ...
            'days': days,    # temporal sampling, number of days since the first observation in the sequence, (T, )
                             # 时间样本，从序列中的第一次观测以来的天数，（T, ）。例如：[0., 5., 10.,15.,...]
            'sample_index': idx,
    ...
    def _get_precomputed_masks(
        ...
        mask_dir, mask_name = dataset_tools.get_mask_sampling_id_hdf5(self.mask_kwargs) # mask_dir：ratio_masked_0.5，mask_name：masks_random_clouds
        ...
        # series
        # intersect_real_cloud_masks：如果设置为 True，则会将随机生成的云遮挡掩码与实际云遮挡掩码相交，以获得更真实的遮挡情况。
    ...
    def _get_data_samples(self) -> Tuple[List[str], Dict[str, List[str]]]:
        ...
        if self.filter_settings is not None and self.filter_settings.type == 'cloud-free' and \
            ...
            # 获取hdf5文件中'path_samples_{name}'里面的路径
    ...
    def _mask_images_with_cloud_coverage_above_p(self, cloud_mask: torch.Tensor) -> torch.Tensor:
        ...
        # 这行代码的作用是将 cloud_mask 中那些在时间序列中遮挡程度超过 self.render_occluded_above_p 的像素标记为 1，表示它们被完全遮挡了。
        # 也就是说，如果 coverage 中某个像素的值大于 self.render_occluded_above_p，那么对应的 cloud_mask 中的像素就会被标记为 1，表示它们被云层完全遮挡。

        # coverage 是一个由 cloud_mask 计算而来的张量，用于表示每个时间步（时间序列的帧）中整个图像上的平均云层覆盖率。
        # 这是通过 torch.mean 函数计算得出的。dim=(1, 2, 3) 参数指定了在哪些维度上进行平均计算。
        # 在这里，(1, 2, 3) 表示在时间维度（第一个维度）以及整个图像的高度和宽度维度上进行平均计算。
        # 因此，coverage 是一个形状为 (T,) 的张量，其中 T 代表时间步的数量，每个元素表示对应时间步的平均云层覆盖率。
        # 接下来的代码行根据 self.render_occluded_above_p 参数，将 cloud_mask 中那些时间步中的云层覆盖率大于该阈值的像素标记为 1，表示它们被认为是完全遮挡的。
        # 换句话说，如果某个时间步的 coverage 大于 self.render_occluded_above_p，则相应时间步的整个图像将被标记为完全遮挡，
        # 因为 cloud_mask 是一个 (T, 1, H, W) 形状的张量，这会在所有高度和宽度上将该时间步的像素值设置为 1。
    ...
    def _subsample_sequence(self, sample: h5py._hl.group.Group, seq_length: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.filter_settings.get('return_valid_obs_only', True):
            ...
            # masks_valid_obs是一个 PyTorch 张量，它包含了二进制掩码，其中值为1表示有效的帧，值为0表示无效的帧。
            # .view(-1)表示将一个原始张量展平为一维的新张量。
        else:
