...
class Trainer:
    ...
    def _resume(self, path: str) -> None:
        ...
        """
        # 该方法用于从预训练的模型权重文件中恢复模型的状态和训练进度。

        if not os.path.isfile(path):
            raise FileNotFoundError(f'No checkpoint found at {path}\n')

        checkpoint = torch.load(path) # 从指定路径加载预训练模型的权重。这将加载包括模型权重、优化器状态等在内的模型状态。
        self.model.load_state_dict(checkpoint['model_state_dict']) # 将加载的模型状态中的模型权重（'model_state_dict'）加载到当前模型中。这是用于替换当前模型的权重以恢复先前训练状态的关键步骤。
        self.optimizer.load_state_dict(checkpoint['optimizer_state_dict']) # 将加载的模型状态中的优化器状态（'optimizer_state_dict'）加载到当前优化器中。这是为了保持优化器在先前训练状态下的参数。

        if self.args.get('load_scheduler_state_dict', True) and 'scheduler_state_dict' in checkpoint:
            # 如果配置参数中指定要加载调度器的状态，且在加载的模型状态中存在调度器状态（'scheduler_state_dict'）。
            # 将加载的模型状态中的调度器状态（'scheduler_state_dict'）加载到当前调度器中。调度器通常用于调整学习率等参数。
            self.scheduler.load_state_dict(checkpoint['scheduler_state_dict'])

        # Extract the last training epoch
        self.epoch = checkpoint['epoch'] + 1
        self.iter = checkpoint['iter']
        self.args.num_epochs += self.epoch # 将训练的总轮数（num_epochs）更新为之前的总轮数加上上次训练的轮数，以确保继续训练。

        # Best validation loss so far
        self.best_loss = checkpoint['best_loss'] # 提取模型训练过程中记录的最佳验证损失。
        self.epoch_best_loss = checkpoint['epoch'] # 提取记录最佳验证损失时的训练轮数。
